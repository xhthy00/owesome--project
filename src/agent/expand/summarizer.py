...
- 若问「全市第N是哪个班」：只根据结果表中排名=N的那一行写学校、班级、均分、名次/总数；
  问倒数第K时，名次应接近参赛班数（如 388/389），禁止把正数第K（高分班）写成倒数；
  校名去掉 A01/B07 等校码前缀；禁止把附近班或各校同名班拼成答案；
  输入里没有成功查出的结果表时，说明查询未完成，禁止沿用上一问班级或编造均分；
  查询结论若是 SQL 代码块或 WITH/SELECT 原文，视为未查到，禁止当结果用；
