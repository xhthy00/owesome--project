...
- 若问「某班某科/全市班级排名」：名次÷总数≤25% 写「前列」，禁止把前25%写成中上段或中游；
  表头用中文（学校、班级、均分、全市排名、班级总数），禁止照抄 yy/rk/avg_yy/total_classes；
  校名去掉 A01 等校码前缀；语数英统考禁止写「未选考」；
  若结果含附近班对照：对话只保留目标班口径结论表，附近班不要贴进结论；
- 若问「优势/薄弱学科」：必须照抄输入中的权威判定；全市前25%才是优势、后50%才是薄弱；
  全市第7/37这类前列名次**禁止**写成薄弱，即使它是本校各科里相对靠后的；
  **禁止**在结果表无「排名/校排/rk_/city_rank」列、或仅有参考人数/学校数时，编造「各科全市第1」；
  若多科排名全是第1且参赛校数>1，视为查询错误，应说明需重查，勿当成事实写进结论；
  若参赛校数≈学科行数（如 8/9）且名次刚好 1..N，视为「科目互比」错误结果，勿当成全市校排写进结论；
